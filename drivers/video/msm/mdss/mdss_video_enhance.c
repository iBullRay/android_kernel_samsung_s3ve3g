...
void lut_tune(int num, unsigned int *pLutTable)
{
	...
	* cmap allocation
	...
	for (i = 0; i < cmap->len; i++) {
		...
		MIPI_OUTP(MDP_BASE + 0x94800 +
#else
		MIPI_OUTP(MDP_BASE + 0x93800 +
#endif
	...
	if (mfd->panel.type == MIPI_CMD_PANEL) {
		...
		out = MIPI_INP(MDP_BASE + 0x90070) & ~((0x1 << 10) | 0x7);
		MIPI_OUTP(MDP_BASE + 0x90070, (mdp_lut_i << 10) | 0x7 | out);
		//mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
	}
...
void sharpness_tune(int num)
{
	...
	MIPI_OUTP(vg_base + 0x8200, mdp4_ss_table_value((int8_t) num, 0));
	MIPI_OUTP(vg_base + 0x8204, mdp4_ss_table_value((int8_t) num, 1));
	//mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}
#endif
int s3c_mdnie_start()
...
static ssize_t accessibility_store(struct device *dev,
			...
			const char *buf, size_t size)
{
#if defined (CONFIG_FB_MSM_MDSS_HIMAX_QHD_PANEL)
	int cmd_value;
	char buffer[MDNIE_COLOR_BLINDE_CMD_LENGTH] = {0,};
	int buffer2[MDNIE_COLOR_BLINDE_CMD_LENGTH/2] = {0,};
	int loop;

	sscanf(buf, "%d %x %x %x %x %x %x %x %x %x", &cmd_value,
		&buffer2[8], &buffer2[7], &buffer2[6], &buffer2[5], &buffer2[4],
		&buffer2[3], &buffer2[2], &buffer2[1], &buffer2[0]);

	for(loop = 0; loop < MDNIE_COLOR_BLINDE_CMD_LENGTH/2; loop++) {
...
// #elif defined (CONFIG_FB_MSM_MDSS_HIMAX_QHD_PANEL)
		// memcpy(&COLOR_BLIND_MDNIE[MDNIE_COLOR_BLINDE_CMD_INDEX],
				// buffer, MDNIE_COLOR_BLINDE_CMD_LENGTH);
// #endif
